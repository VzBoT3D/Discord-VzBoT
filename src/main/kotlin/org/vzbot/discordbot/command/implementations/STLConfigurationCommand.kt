...
class STLConfigurationCommand: Command("stlconfig", cmdData , false) {
    ...
    override fun execute(member: Member, event: SlashCommandInteractionEvent) {
        ...
        if (STLFinderManager.isConfiguring(member)) return event.replyEmbeds(defaultEmbed("You are already configuring the stl finder")).queue()
        ...
        event.replyEmbeds(embed).addActionRow(Button.primary("c_new_chart", "Create new chart"), Button.primary("c_view_charts", "View existing charts"))
            ...
            .addActionRow(Button.danger("c_cancel", "Cancel")).queue { it ->
                it.retrieveOriginal().queue { STLFinderManager.addConfiguring(member, it) }
            }
    }
}