...
class RegisterSerialCommand : Command("register", commandData, true) {
    ...
    override fun execute(member: Member, event: SlashCommandInteractionEvent) {
        ...
        if (description.length >= 1024 || media.length >= 1024) {
            return event.reply("Please only use 1024 characters for the description and media").queue()
        }
        ...
        val announceChannel = event.jda.getGuildById(829828765512106054)!!
            .getTextChannelById(VzBot.configFileManager.getSerialAnnouncementChannelID())
            ?: error("Serial announcements channel was not found xD")

        val fileToSend = File(LocationGetter().getLocation().absolutePath + "/VZBoT/plates/${registration.id}.stl")
        val lowerPlate =
            if (registration.id < 100) {
                File(LocationGetter().getLocation().absolutePath + "/VZBoT/serial_plate_bottom.stl")
            } else {
                File(
                    LocationGetter().getLocation().absolutePath + "/VZBoT/Serial_plate_bottom-100-999.stl",
                )
            }
        ...
        event.replyEmbeds(returnEmbed.build())
            .addFiles(FileUpload.fromData(fileToSend), FileUpload.fromData(lowerPlate)).queue()
    }
}
