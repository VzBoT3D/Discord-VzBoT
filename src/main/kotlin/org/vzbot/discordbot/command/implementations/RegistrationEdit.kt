...
class RegistrationEdit: Command("registrationedit", cmdData, false) {
    ...
    override fun execute(member: Member, event: SlashCommandEvent) {
        ...
        val newDescription = if (event.getOption("description") != null) event.getOption("description")!!.asString else registration.description
        val newMedia = if (event.getOption("media") != null) event.getOption("media")!!.asString else registration.mediaURL
        val newCountry = if (event.getOption("country") != null) event.getOption("country")!!.asString else registration.country
